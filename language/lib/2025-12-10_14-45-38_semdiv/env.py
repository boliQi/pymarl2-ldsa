...
class ForagingEnv(Env):
    ...
    '''
    2025-12-10_14-48-01
    Human players may prefer to collect the food that has the minimal sum of distances from both agents.
    '''
    def llm1(self) -> float:
        sum_dists = {food: self.agent_food_distance("1", food) + self.agent_food_distance("2", food) for food in self.foods_position}
        min_sum = min(sum_dists.values())
        max_possible_sum = 20.0
        return (1 - min_sum / max_possible_sum) * 0.1
